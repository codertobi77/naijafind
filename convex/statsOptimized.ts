...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Build map for O(1) lookups during aggregation
    ...
    // Single pass over suppliers to aggregate stats (O(S))
    for (const s of suppliers) {
      const stats = s.category ? categoryStats.get(s.category) : null;
      if (stats) {
        stats.total++;
    ...
    // Return stats in the order of categories
    return categories.map((cat) => ({
      name: cat.name,
      ...(categoryStats.get(cat.name) || { total: 0, approved: 0, featured: 0, verified: 0 }),
    }));
  },
});
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single pass over suppliers (O(S))
    ...
    // Single pass over categories (O(C))
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Category breakdown with counts (O(C + S))
