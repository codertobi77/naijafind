...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Use Map for O(1) lookups and single-pass aggregation O(C + S)
    ...
    for (const s of suppliers) {
      ...
      if (catName && statsMap.has(catName)) {
        const stat = statsMap.get(catName);
        stat.total++;
        if (s.approved) stat.approved++;
        if (s.featured) stat.featured++;
        if (s.verified) stat.verified++;
      }
    }
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single-pass aggregation for multiple collections to reduce CPU cycles
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Count active categories and initialize categoryCounts
    ...
    // Single-pass aggregation for multiple metrics on approved suppliers
    ...
    for (const s of approvedSuppliers) {
      ...
      const catName = s.category;
      if (catName && categoryCounts[catName] !== undefined) {
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Count products with a single loop
