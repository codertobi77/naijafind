...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize database fetches to reduce total latency
    const [categories, suppliers] = await Promise.all([
      ctx.db.query("categories").take(1000),
      ctx.db.query("suppliers").take(10000)
    ]);

    // Use a Map for O(C + S) aggregation instead of O(C * S) nested filtering
    ...
    // Initialize map with all categories to ensure they appear in results even with 0 counts
    for (const cat of categories) {
      categoryStats.set(cat.name, { total: 0, approved: 0, featured: 0, verified: 0 });
    }
    ...
    // Convert Map back to the expected array format (O(C))
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize suppliers and categories fetches
    ...
    // Single-pass aggregation for featured, verified, and ratings
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    let totalRating = 0;
    let ratedCount = 0;
    
    // Initialize category counts map with 0 for all active categories
    ...
    for (const s of approvedSuppliers) {
      ...
      // Increment category count if supplier belongs to an active category
