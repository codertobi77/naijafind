...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single-pass aggregation of supplier stats by category
    const categoryStatsMap = new Map<string, { total: number; approved: number; featured: number; verified: number }>();
    
    for (const s of suppliers) {
      if (!s.category) continue;
      const stats = categoryStatsMap.get(s.category) || { total: 0, approved: 0, featured: 0, verified: 0 };
      stats.total++;
      if (s.approved) stats.approved++;
      if (s.featured) stats.featured++;
      if (s.verified) stats.verified++;
      categoryStatsMap.set(s.category, stats);
    }

    // Build detailed stats from categories
    return categories.map((cat) => {
      const stats = categoryStatsMap.get(cat.name) || { total: 0, approved: 0, featured: 0, verified: 0 };
      return {
        name: cat.name,
        ...stats
      };
