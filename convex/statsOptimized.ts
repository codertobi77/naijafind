...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Optimization: Parallelize fetches and use single-pass aggregation
    ...
    // Initialize stats map for O(1) lookup
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Optimization: Single-pass aggregation for suppliers
    ...
    // Optimization: Single-pass aggregation for users
    let totalSuppliersAsUsers = 0;
    for (const u of allUsers) {
      if (u.user_type === 'supplier') totalSuppliersAsUsers++;
    }
    
    // Optimization: Single-pass aggregation for products
    let activeProducts = 0;
    for (const p of allProducts) {
      if (p.status === 'active') activeProducts++;
    }
    
    // Optimization: Single-pass aggregation for categories
    let activeCategories = 0;
    for (const c of allCategories) {
      if (c.is_active !== false) activeCategories++;
    }
    
    // Optimization: Single-pass aggregation for claims
    let pendingClaims = 0;
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Initialize category counts with all active categories
    const categoryCounts: Record<string, number> = {};
    for (const cat of categories) {
      categoryCounts[cat.name] = 0;
    }

    // Optimization: Single-pass aggregation for multiple stats
    let featuredCount = 0;
    let verifiedCount = 0;
    let totalRating = 0;
    let ratedCount = 0;
    ...
    return {
      totalSuppliers,
      featuredSuppliers: featuredCount,
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Optimization: Single-pass for active products count
    let activeProducts = 0;
