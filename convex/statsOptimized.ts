 ...
 * Optimized to O(C + S) using single-pass aggregation
 */
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Initialize stats Map with all categories to ensure 0-counts are included
    ...
    // Single-pass aggregation: O(S)
    for (const supplier of suppliers) {
      const stats = statsMap.get(supplier.category);
      if (stats) {
        stats.total++;
        if (supplier.approved) stats.approved++;
        if (supplier.featured) stats.featured++;
        if (supplier.verified) stats.verified++;
      }
    }
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single-pass aggregation for suppliers
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetches for suppliers and categories
    ...
    // Initialize category counts map with all active categories
    const categoryCounts: Record<string, number> = {};
    for (const cat of categories) {
      categoryCounts[cat.name] = 0;
    }

    // Single-pass aggregation for approved suppliers
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    let totalRating = 0;
    let ratedCount = 0;
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Parallelize fetches for reviews and products
