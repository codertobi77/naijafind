...
export const _getCategoryStatsInternal = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetching categories and suppliers (O(1) RTT)
...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetching categories and suppliers
    const [categories, suppliers] = await Promise.all([
      ctx.db.query("categories").take(1000),
      ctx.db.query("suppliers").take(10000)
    ]);
    
    // Pre-calculate counts per category using a single-pass Map (O(S))
    ...
    for (const s of suppliers) {
      ...
      const stats = categoryAggregates.get(s.category) || { total: 0, approved: 0, featured: 0, verified: 0 };
      stats.total++;
      if (s.approved) stats.approved++;
      if (s.featured) stats.featured++;
      if (s.verified) stats.verified++;
      categoryAggregates.set(s.category, stats);
    }
    
    // Build detailed stats as array by mapping categories (O(C))
    const stats = categories.map((cat) => {
      const counts = categoryAggregates.get(cat.name) || { total: 0, approved: 0, featured: 0, verified: 0 };
      return {
        name: cat.name,
        ...counts
      };
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Fetch all tables in parallel to minimize RTT
    const [
      allSuppliers,
      allUsers,
      allReviews,
    ...
    // Perform all supplier-related counts in a single pass (O(S))
    ...
    // Count users in a single pass (O(U))
    ...
    // Count products in a single pass (O(P))
    ...
    // Count categories in a single pass (O(C))
    let activeCategories = 0;
    for (const c of allCategories) {
      if (c.is_active !== false) activeCategories++;
    }
    
    // Count claims in a single pass (O(Claims))
    let pendingClaims = 0;
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetching suppliers and categories (O(1) RTT)
    ...
    // Process all approved suppliers in a single pass (O(S))
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Parallelize fetching reviews and products (O(1) RTT)
    const [reviews, products] = await Promise.all([
      ctx.db.query("reviews").withIndex("supplierId", (q) => q.eq("supplierId", args.supplierId)).collect(),
      ctx.db.query("products").withIndex("supplierId", (q) => q.eq("supplierId", args.supplierId)).collect()
    ]);
...
export const _adminStatsInternal = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize all table fetches (O(1) RTT)
    const [
      allSuppliers,
      allUsers,
      allReviews,
    ...
    // Single-pass supplier aggregation (O(S))
    let pendingSuppliers = 0;
    let approvedSuppliers = 0;
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    ...
    // Single-pass user aggregation (O(U))
    let totalSuppliersAsUsers = 0;
    for (const u of allUsers) {
      if (u.user_type === 'supplier') totalSuppliersAsUsers++;
    }

    // Single-pass product aggregation (O(P))
    let activeProducts = 0;
    for (const p of allProducts) {
      if (p.status === 'active') activeProducts++;
    }

    // Single-pass category aggregation (O(C))
    let activeCategories = 0;
    for (const c of allCategories) {
      if (c.is_active !== false) activeCategories++;
    }

    // Single-pass claim aggregation (O(Claims))
    let pendingClaims = 0;
    let approvedClaims = 0;
    for (const c of allClaims) {
      if (c.status === 'pending') pendingClaims++;
