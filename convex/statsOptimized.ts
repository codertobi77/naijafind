...
export const getSupplierCount = query({
  ...
  handler: async (ctx) => {
    ...
    // Using .count() is more efficient than fetching documents
    return await ctx.db
      .query("suppliers")
...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Use Map for single-pass aggregation O(C + S) instead of O(C * S)
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Parallelize all primary database fetches for better performance
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Category counts initialized with 0 for all active categories
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Single pass for active products
    let activeProducts = 0;
