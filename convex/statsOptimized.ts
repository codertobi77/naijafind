...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Build detailed stats efficiently using a Map for aggregation - O(C + S) complexity
    ...
    for (const s of suppliers) {
      if (s.category && statsMap.has(s.category)) {
        const current = statsMap.get(s.category)!;
        current.total++;
    ...
    const stats = categories.map((cat) => {
      const data = statsMap.get(cat.name)!;
      return {
        name: cat.name,
        ...data
      };
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Parallelize data fetching for all tables - Reduces RTT latency
    const [
      allSuppliers,
      allUsers,
      allReviews,
    ...
    // Single-pass aggregation for suppliers - O(N) instead of multiple O(N) filter passes
    ...
    // Single-pass aggregation for other metrics
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Parallelize data fetching for specific supplier
