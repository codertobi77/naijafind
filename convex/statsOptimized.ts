...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single pass over suppliers to aggregate counts O(S)
    for (const s of suppliers) {
      const stats = s.category ? statsMap.get(s.category) : null;
      if (stats) {
        stats.total++;
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single pass over approved suppliers to calculate all metrics
    ...
    return {
      ...
      averageRating: Math.round(averageRating * 100) / 100, // Standardized to 2 decimal places
      categoryCounts,
    };
  },
});
