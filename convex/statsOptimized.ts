...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    for (const s of suppliers) {
      ...
      const entry = statsMap.get(s.category) || { total: 0, approved: 0, featured: 0, verified: 0 };
      entry.total++;
      if (s.approved) entry.approved++;
      if (s.featured) entry.featured++;
      if (s.verified) entry.verified++;
      statsMap.set(s.category, entry);
    }

    return categories.map((cat) => {
      const s = statsMap.get(cat.name) || { total: 0, approved: 0, featured: 0, verified: 0 };
      return {
        name: cat.name,
        total: s.total,
        approved: s.approved,
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    for (const s of allSuppliers) {
      if (s.approved) approvedSuppliersCount++; else pendingSuppliers++;
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetching approved suppliers and active categories
    ...
    // Single-pass aggregation over approved suppliers
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Verify user and fetch supplier in parallel
    const [user, supplier] = await Promise.all([
      ctx.db.query("users").withIndex("email", (q) => q.eq("email", identity.email ?? "")).first(),
    ...
    // Count reviews and products in parallel
