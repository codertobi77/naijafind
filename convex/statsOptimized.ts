...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetches for efficiency
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single pass for supplier stats
    ...
    // Other counts with single passes where beneficial
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Category breakdown with counts - initialize all active categories with 0
    const categoryCounts: Record<string, number> = {};
    for (const cat of categories) {
      categoryCounts[cat.name] = 0;
    }
    
    // Single pass for all stats (O(S))
