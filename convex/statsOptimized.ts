...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Build detailed stats using single-pass aggregation (O(C + S) complexity)
    // Map to store running counts for each category
    const statsMap = new Map<string, { total: number, approved: number, featured: number, verified: number }>();
    
    // Initialize Map with all active categories
    for (const cat of categories) {
    ...
    // Convert Map back to expected array format
    return categories.map(cat => ({
      name: cat.name,
      ...(statsMap.get(cat.name) || { total: 0, approved: 0, featured: 0, verified: 0 }),
    }));
  },
});
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single-pass claims aggregation
    let pendingClaims = 0;
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    for (const s of approvedSuppliers) {
      ...
      const currentCount = categoryCountsMap.get(s.category);
      if (currentCount !== undefined) {
        categoryCountsMap.set(s.category, currentCount + 1);
      }
    }
