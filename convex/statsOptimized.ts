 ...
 * Optimized: Uses single-pass aggregation O(C + S) instead of O(C * S)
 */
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Initialize stats map with all categories to ensure they appear even if empty
    ...
    // Build detailed stats in a single pass over suppliers
    ...
    // Convert Map back to array format expected by the frontend
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Count suppliers efficiently in a single pass
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Consolidate supplier metrics and category counts in a single pass
    ...
    const averageRating = ratedCount > 0 ? totalRating / ratedCount : 0;
    const categoryCounts = Object.fromEntries(categoryCountsMap);
