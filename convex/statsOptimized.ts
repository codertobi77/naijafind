...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Optimization: Use a Map for O(1) category lookups and single-pass aggregation
    // Complexity reduced from O(C * S) to O(C + S)
    ...
    for (const s of suppliers) {
      ...
      if (catName && categoryStatsMap.has(catName)) {
        const stats = categoryStatsMap.get(catName)!;
        stats.total++;
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Optimization: Single-pass supplier aggregation
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Optimization: Parallelize fetching of suppliers and categories
    ...
    // Optimization: Use a Map for O(C + S) category counting instead of O(C * S)
    const categoryCounts: Record<string, number> = {};
    // Initialize with all active categories
    for (const cat of categories) {
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Optimization: Parallelize fetching of reviews and products
    ...
    // Optimization: Single-pass product status check
    let activeProducts = 0;
