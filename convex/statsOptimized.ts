...
export const _getCategoryStatsInternal = query({
  args: {},
  handler: async (ctx) => {
    // Optimization: Parallelize fetching categories and suppliers
...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Optimization: Parallelize database fetches to reduce total latency
    const [categories, suppliers] = await Promise.all([
      ctx.db.query("categories").take(1000),
      ctx.db.query("suppliers").take(10000),
    ]);
    
    // Optimization: Use a Map for O(1) lookup during aggregation,
    // reducing overall complexity from O(C * S) to O(C + S)
    const categoryStatsMap = new Map<string, { total: number; approved: number; featured: number; verified: number }>();
    
    // Initialize Map with all categories to ensure all are included in results
    for (const cat of categories) {
    ...
    // Map results back to categories while preserving original order
    return categories.map((cat) => ({
      name: cat.name,
      ...categoryStatsMap.get(cat.name)!,
    }));
  },
});
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Optimization: Parallelize all primary database fetches to minimize total wait time
    const [
      allSuppliers,
      allUsers,
      allReviews,
    ...
    // Optimization: Use single-pass aggregation loops instead of multiple .filter() calls
    // to reduce CPU overhead from O(M * N) to O(N) for each dataset.
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Optimization: Parallelize fetching approved suppliers and active categories
    ...
    // Optimization: Single-pass aggregation for counts and ratings
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    let totalRating = 0;
    let ratedCount = 0;
    
    // Category breakdown optimization: use a Map for O(1) lookup during aggregation
