...
export const _getCategoryStatsInternal = query({
  ...
  handler: async (ctx) => {
    ...
    // Parallelize with suppliers fetch
    const [categories, suppliers] = await Promise.all([
      ctx.db.query("categories").take(1000),
      ctx.db.query("suppliers").take(10000) // Limit to prevent timeout
    ]);
    ...
    // Count suppliers - Single pass O(S)
...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetches O(1) round trips
    const [categories, suppliers] = await Promise.all([
      ctx.db.query("categories").take(1000),
      ctx.db.query("suppliers").take(10000)
    ]);
    
    // Build detailed stats Map for O(1) access
    ...
    // Initialize Map with all categories O(C)
    for (const cat of categories) {
    ...
    // Single pass through suppliers O(S)
    for (const s of suppliers) {
      const stats = categoryStats.get(s.category);
      if (stats) {
        stats.total++;
    ...
    // Convert back to expected array format O(C)
    // Total complexity O(C + S) instead of O(C * S)
    return categories.map((cat) => ({
      name: cat.name,
      ...categoryStats.get(cat.name)!
    }));
  },
});
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Parallelize all database fetches O(1) round trips
    const [
      allSuppliers,
      allUsers,
      allReviews,
    ...
    // Count suppliers efficiently with single pass O(S)
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single pass aggregation for multiple metrics O(S)
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    let totalRating = 0;
    let ratedCount = 0;
    ...
    // Pre-initialize category counts for all active categories O(C)
    for (const cat of categories) {
    ...
    for (const s of approvedSuppliers) {
      ...
      // Increment category count if it's an active category
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Single pass to count active products
    let activeProducts = 0;
...
export const _adminStatsInternal = query({
  ...
  handler: async (ctx) => {
    ...
    // Count suppliers efficiently with single pass
    let pendingSuppliers = 0;
    let approvedSuppliers = 0;
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    ...
    // Count users single pass
    let totalSuppliersAsUsers = 0;
    for (const u of allUsers) {
      if (u.user_type === 'supplier') totalSuppliersAsUsers++;
    }

    // Count products single pass
    let activeProducts = 0;
    for (const p of allProducts) {
      if (p.status === 'active') activeProducts++;
    }

    // Count categories single pass
    let activeCategories = 0;
    for (const c of allCategories) {
      if (c.is_active !== false) activeCategories++;
    }

    // Count claims single pass
    let pendingClaims = 0;
    let approvedClaims = 0;
    for (const c of allClaims) {
      if (c.status === 'pending') pendingClaims++;
