...
export const getDetailedCategoryStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Build a map of category statistics in O(S) time
    const statsMap = new Map<string, { total: number; approved: number; featured: number; verified: number }>();
    
    for (const s of suppliers) {
      if (!s.category) continue;
      const current = statsMap.get(s.category) || { total: 0, approved: 0, featured: 0, verified: 0 };
      current.total++;
      if (s.approved) current.approved++;
      if (s.featured) current.featured++;
      if (s.verified) current.verified++;
      statsMap.set(s.category, current);
    }
    
    // Map categories to their stats in O(C) time
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Initialize category counts with 0 for all active categories
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Single-pass review aggregation
    let totalRating = 0;
