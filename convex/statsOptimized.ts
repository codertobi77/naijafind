...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize category and supplier fetches
    const [categories, suppliers] = await Promise.all([
      ctx.db.query("categories").take(1000),
      ctx.db.query("suppliers").take(10000),
    ]);
    
    // O(C+S) aggregation using a Map to avoid O(C*S) nested filter
    ...
    // Aggregate in a single pass over suppliers
    ...
    // Map back to the required array format
...
export const getAdminStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Fetch all primary data in parallel to reduce total round-trip time
    ...
    // Single-pass loop over suppliers for multiple metrics (O(N) vs multiple O(N) passes)
    ...
    for (const s of allSuppliers) {
      if (s.approved) approvedSuppliers++; else pendingSuppliers++;
    ...
    // Single-pass loops for other data sets
...
export const getHomepageStats = query({
  ...
  handler: async (ctx) => {
    ...
    // Single-pass calculation for featured, verified and average rating
    let featuredSuppliers = 0;
    let verifiedSuppliers = 0;
    let totalRating = 0;
    let ratedCount = 0;
    
    // O(C+S) aggregation for category counts using a Map
    ...
    for (const s of approvedSuppliers) {
      ...
      // Update category count if the supplier belongs to an active category
      if (s.category && countsMap.has(s.category)) {
        countsMap.set(s.category, countsMap.get(s.category)! + 1);
      }
    }

    const averageRating = ratedCount > 0 ? totalRating / ratedCount : 0;
    const categoryCounts = Object.fromEntries(countsMap);
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Verify user owns this supplier or is admin - Fetch user and supplier in parallel
    ...
    // Single-pass count for products
    let activeProducts = 0;
