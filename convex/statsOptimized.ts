...
export const getDetailedCategoryStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize database fetches to reduce latency (O(1) round-trip time)
    ...
    // Single-pass aggregation: O(S) instead of O(C * S)
    ...
    // Return stats in the expected array format
...
export const getHomepageStats = query({
  args: {},
  handler: async (ctx) => {
    // Parallelize fetches for O(1) round-trip time
    ...
    // Single-pass aggregation: O(S) instead of multiple O(S) filters and O(C * S) nested loops
...
export const getSupplierStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Single-pass product status counting
    let activeProducts = 0;
