...
export const searchProducts = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Pre-calculate mapping of category to its most relevant product for supplier scoring
    const categoryToProduct = new Map<string, ScoredProduct>();
    const productCategories: string[] = [];

    for (const p of scored) {
      if (p.category && p.category.trim().length > 0) {
        productCategories.push(p.category.toString());
        // Store the first (most relevant) product for each category
        if (!categoryToProduct.has(p.category)) {
