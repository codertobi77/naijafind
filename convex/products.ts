...
export const searchProducts = action({
  ...
  handler: async (ctx, args) => {
    ...
    // PERFORMANCE OPTIMIZATION (Bolt): Use Map for O(1) product lookup by category
    ...
    // PERFORMANCE OPTIMIZATION (Bolt): Parallelize supplier fetches for all categories
    // This significantly reduces total latency by avoiding sequential RTTs
    await Promise.all(
        ...
        try {
          ...
          if (candidates.length > 0) {
            ...
            if (productSuppliers.length > 0) {
              // PERFORMANCE OPTIMIZATION (Bolt): Use O(1) Map lookup instead of O(N) .find()
    ...
    for (const p of scored) {
      ...
      if (list.length === 0) {
        ...
        // Removed console.log in hot loop for performance
      }
