...
/**
 * Static constants for keyword extraction to avoid re-calculation in every search.
 */
...
export const searchProducts = action({
  ...
  handler: async (ctx, args) => {
    ...
    const minPrice = args.minPrice ?? -Infinity;
    const maxPrice = args.maxPrice ?? Infinity;

    // Optimized single-pass filter and scoring
    const scored: ScoredProduct[] = [];
    const categoriesSet = new Set<string>(inferredCategories);
    const categoryToFirstProduct = new Map<string, any>();
    ...
    for (const p of rawProducts) {
      ...
      // Minimum relevance threshold for queries
      if (hasQuery && score < 10) continue;

      const scoredProduct = { ...p, _score: score, _suppliers: null };
      scored.push(scoredProduct);

      // Collect categories for mapping
      if (p.category) {
        const cat = p.category.toString();
        if (cat.trim().length > 0) {
          categoriesSet.add(cat);
          if (!categoryToFirstProduct.has(cat)) {
            categoryToFirstProduct.set(cat, scoredProduct);
          }
        }
      }
    }

    // Suppliers per CATEGORY - Parallelized
      ...
      try {
        ...
        if (candidates.length > 0) {
          ...
          if (productSuppliers.length > 0) {
            const matchingProduct = categoryToFirstProduct.get(cat);
