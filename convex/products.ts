...
const CATEGORY_KEYWORDS_ENTRIES = Object.entries(CATEGORY_KEYWORDS);
...
function inferCategoriesFromKeywords(keywords: string[]): string[] {
  ...
  for (const keyword of keywords) {
    ...
    for (const [knownKeyword, categories] of CATEGORY_KEYWORDS_ENTRIES) {
...
export const searchProducts = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Optimized single-pass filtering and scoring
    ...
    for (const p of rawProducts) {
      ...
      // If all checks pass, add to scored list
      const scoredProduct = p as ScoredProduct;
      scoredProduct._score = score;
      scoredProduct._suppliers = null;
    ...
    // Pre-calculate a map of category to representative product for faster lookups
    ...
    // Fetch suppliers for all categories in parallel to reduce total round-trip time
    await Promise.all(
