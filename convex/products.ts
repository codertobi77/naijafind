...
function isServiceCategory(category: string | undefined): boolean {
  ...
  // First check cache with raw string to bypass normalization if possible
  const rawCached = serviceCategoryCache.get(category);
  if (rawCached !== undefined) return rawCached;
...
export const searchProducts = action({
  ...
  handler: async (ctx, args) => {
    ...
    // CONSOLIDATED FILTERING AND SCORING PASS
    for (const p of rawProducts) {
      ...
      // 3. Calculate enhanced relevance score with category inference
      ...
      // 5. For queries, require a minimum relevance score to filter out unrelated products
      ...
      // Pre-index by category for supplier scoring phase
      if (p.category && !categoryToProduct.has(p.category)) {
        categoryToProduct.set(p.category, scoredProduct);
      }
    }
          ...
          if (candidates.length > 0) {
            ...
            if (productSuppliers.length > 0) {
              ...
              const scoredSuppliers = productSuppliers.map((s: any) => {
                // Get the product that triggered this category search (O(1) lookup)
    ...
    // Optimize assembly with a lowercase-keyed map for case-insensitive lookups
    ...
    for (const p of scored) {
      ...
      if (p.category && typeof p.category === "string") {
        ...
        // If no exact match, try case-insensitive match (O(1) using the pre-calculated map)
        if (list.length === 0) {
          const prodCatLower = p.category.toLowerCase().trim();
          list = lowercaseCategoryToSuppliers.get(prodCatLower) || [];
          if (list.length > 0) {
            console.log(`Case-insensitive match found for category: "${p.category}"`);
          }
        }
      }
