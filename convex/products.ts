...
export const searchProducts = action({
  ...
  handler: async (ctx, args) => {
    ...
    // OPTIMIZATION: Combine multiple filtering passes and category extraction into a single loop
    const queryLower = hasQuery ? args.q!.toLowerCase().trim() : "";
    const productCategoriesSet = new Set<string>();
    const categoryToProduct = new Map<string, any>(); // $O(1)$ lookup for supplier scoring

    const filteredScored: ScoredProduct[] = [];
    for (const p of scored) {
      ...
      // 3. Relevance score and additional filters
      ...
      // Collect categories and first product instance for mapping
      if (p.category) {
    ...
    // OPTIMIZATION: Parallelize category-based supplier queries to reduce total RTT
      ...
      try {
        ...
        if (candidates.length > 0) {
          ...
          if (productSuppliers.length > 0) {
            // OPTIMIZATION: Use pre-calculated categoryToProduct Map for $O(1)$ lookup
    ...
    // OPTIMIZATION: Pre-calculate lowercase category map for $O(1)$ case-insensitive matching
    ...
    for (const p of scored) {
      ...
      if (p.category && typeof p.category === "string") {
        // Try exact match first, then case-insensitive via the pre-calculated Map
        list = categoryToSuppliers.get(p.category) ||
               lowercaseCategoryToSuppliers.get(p.category.toLowerCase().trim()) ||
               [];
      }
