...
export const getSearchSuggestions = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Use internal queries to fetch data server-side in parallel for better performance
...
export const searchSuggestionsWithQuery = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Get all data in parallel using internal queries to minimize round-trips
    const [suppliers, products, categories] = await Promise.all([
    ...
    // Optimization: Build a Map of category to suppliers to avoid N+1 query problem
    ...
    suppliers.forEach((s: any) => {
      if (s.category) {
        ...
        if (list.length < 10) { // Keep same limit as original getSuppliersByCategory
          list.push(s);
        }
      }
    });

    // Add suppliers from matched product categories using in-memory Map (O(1) lookup)
    for (const category of matchedProductCategories) {
      ...
      categorySuppliers.forEach((s: any) => {
        if (!s.business_name) return;
        // Avoid duplicates - we'll handle overall deduplication efficiently at the end
        suggestions.push({
          text: s.business_name,
    ...
    // Efficient deduplication using a Set (O(N) instead of O(N^2))
    const seenSuggestions = new Set<string>();
    const sortedSuggestions = [];

    for (const sug of suggestions) {
      const lowerText = sug.text.toLowerCase();
      if (!seenSuggestions.has(lowerText)) {
        seenSuggestions.add(lowerText);
        sortedSuggestions.push(sug);
        if (sortedSuggestions.length >= limit) break;
      }
    }
