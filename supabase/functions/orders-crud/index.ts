...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { order_number, total_amount, status } = body
      const { error } = await supabaseClient
        .from('orders')
        .insert([{ order_number, total_amount, status, supplier_id: supplierProfile.id }])
    ...
    if (req.method === 'PUT') {
      ...
      const body = await req.json()
      const { id, order_number, total_amount, status } = body
      const { error } = await supabaseClient
        .from('orders')
        .update({ order_number, total_amount, status })
        .eq('id', id)
